...
def run_DQN(RL1,RL2):
    ...
    while episode < max_step:
        ...
        while True:
            ...
            for i in range(redN):   #计算奖赏
               ...
               # env.sc[i]-=step*0.02
                ...
                #   c 保存所有的参数到经验池
                if np.random.uniform() < 1 or env.sc[i]<0:    #通过条件调整经验池中各样本的比重
            ...
            if env.redN <5 or env.blueN<1 or train_step > 15000: #红方小于5个，蓝方小于1个，训练12000轮 退出循环
                ...
                print('episode:[',episode,']reward:',ep_reward,'remaining:[',env.redN,',',env.blueN,']',
                      'UsedSteps', '[', step, ']', 'memory_counter', '[', RL.memory_counter, ']')
                ...
                # 保存网络参数
                if (episode+2)%10 == 0 or (episode+2)%10 == 1:# or max_reward <= ep_reward: #每训练50的倍数或训练奖赏大于当前最大奖赏
                    max_reward = ep_reward
                    RL.stor_DQN(episode%2)                       #保存网络
        ...
        if (episode+4)%50==0:
            ...
            step_1=0
            step_2=0
            # RL1.memory_counter=0
            # RL2.memory_counter=0
...
if __name__ == "__main__":
    ...
    sess = tf.Session()        #定义会话对象
    with tf.variable_scope('dueling1'):  #duelingDQN
        dueling_DQN1 = RL_brain.DuelingDQN(
            n_actions=env.n_actions, n_features=env.n_features, memory_size=100000,
            replace_target_iter=1000,e_greedy=0.7,
            e_greedy_increment=0.1, sess=sess, dueling=True, output_graph=False)
        #动作空间维度，状态空间维度，记忆库，贪婪系数0.05，句柄，dueling 开，输出图 关

    with tf.variable_scope('dueling2'):  #duelingDQN
        dueling_DQN2 = RL_brain_2.DuelingDQN(
            n_actions=env.n_actions, n_features=env.n_features, memory_size=100000,
            replace_target_iter=1000,e_greedy=0.7,
            e_greedy_increment=0.1, sess=sess, dueling=True, output_graph=False)
        #动作空间维度，状态空间维度，记忆库，贪婪系数0.05，句柄，dueling 开，输出图 关
    sess.run(tf.global_variables_initializer())  #网络初始化
    dueling_DQN1.load_DQN(0)
    dueling_DQN2.load_DQN(1)
    run_DQN(dueling_DQN1,dueling_DQN2)
